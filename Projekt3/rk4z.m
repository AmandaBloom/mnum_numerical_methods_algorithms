function [tt, X1, X2, T, err, hh] = rk4z(xs1,xs2,h0,b)
%
%   AUTOR
...
dx1 = @(x1,x2)x2+x1*(0.3-(x1)^2-(x2)^2);
dx2 = @(x1,x2)-x1+x2*(0.3-(x1)^2-(x2)^2);
...
while t+h ~= b
    ...
    if(mod(n,2) == 1)
        ...
        x1d=X1(end)+(1/12)*h*(k_1_1d+2*k_2_1d+2*k_3_1d+k_4_1d);
        x2d=x2(end)+(1/12)*h*(k_1_2d+2*k_2_2d+2*k_3_2d+k_4_2d);
    ...
    if(mod(n,2) == 0)
        ...
        x1=x1(end)+(1/6)*h*(k_1_1+2*k_2_1+2*k_3_1+k_4_1);
        x2=x2(end)+(1/6)*h*(k_1_2+2*k_2_2+2*k_3_2+k_4_2);
        X1=[X1; x1];
        X2=[X2; x2];
